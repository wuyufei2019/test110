...
import com.cczu.model.ztzyaqgl.dao.AqglDtzyDao;
...
@Service("ztAqglDtzyService")
public class AqglDtzyService {
	
	@Resource
	private AqglDtzyDao dtzyDao;
	@Resource
	...
	//添加动土作业
	public void addInfo(ZTAQGL_Dtzy entity) {
		dtzyDao.save(entity);
	}

	//修改动土作业
	public void updateInfo(ZTAQGL_Dtzy entity) {
	...
	//查找动土作业
	public ZTAQGL_Dtzy find(long id) {
		return dtzyDao.find(id);
	}

	public Map<String, Object> dataGrid(Map<String, Object> mapData) {

		List<Map<String, Object>> list = dtzyDao.dataGrid(mapData);
		int getTotalCount = dtzyDao.getTotalCount(mapData);
	...
	public Map<String, Object> findInforById(long id) {
		...
		Map<String, Object> map=dtzyDao.findInforById(id);
		return map;
	}
	 ...
	 * @return
	 */
	public ZTAQGL_Dtzy findById(long id) {
		
		ZTAQGL_Dtzy gczy=dtzyDao.find(id);
		return gczy;
	}
	...
	public void deleteInfo(long id) {
		...
		dtzyDao.deleteInfo(id);
	}
	
	//安全措施datagrid
	public Map<String, Object> aqscdataGrid(Map<String, Object> mapData) {

		List<Map<String, Object>> list = dtzyDao.aqcsList(mapData);
		int getTotalCount = dtzyDao.aqcsCount();
		Map<String, Object> map = new HashMap<String, Object>();
		map.put("rows", list);
		map.put("total", getTotalCount);
		return map;
	...
	public Map<String,Object> findallById(long id) {
		return dtzyDao.findallById(id);
	}

	//根据id查找全部详情
	public List<Map<String,Object>> findallWlfIds(String ids) {
		return dtzyDao.findallWlfIds(ids);
	}
	
	//导出excel
	public void exportExcel(HttpServletResponse response,Map<String, Object> mapData) {
		String fileName="动土作业安全作业证表.xls";
		List<Map<String, Object>> list=dtzyDao.getExport(mapData);
		String[] title={"企业名称","作业证编号","申请人","申请时间","申请单位","动土作业级别","动土方式","动土地点","动土时间起","动土时间止",
				"动土作业负责人","动土人","涉及的其他特殊作业","危害辨识","分析人","安全措施编制人","生产单位负责人","监火人","动土初审人","实施安全教育人","申请单位意见","申请单位负责人","确认时间",
				"安全管理部门意见","安全管理部门负责人","确认时间","动土审批人意见","动土审批人","审批时间","验收人","验收时间"};  
	...
	public Map<String, Object> getExportWord(long id,String webAddress){
		Map<String, Object> map=new HashMap<String, Object>();
		Map<String, Object> mapret = dtzyDao.findallById(id);
		
		map.put("m5", mapret.get("m5")==null?"年  月  日  时  分": DateUtils.formatDate(Timestamp.valueOf(mapret.get("m5").toString()), "yyyy年MM月dd日HH时mm分"));
		map.put("m6", mapret.get("m6")==null?"年  月  日  时  分": DateUtils.formatDate(Timestamp.valueOf(mapret.get("m6").toString()), "yyyy年MM月dd日HH时mm分"));
		map.put("m19_7", mapret.get("m19_7")==null?"年  月  日  时  分": DateUtils.formatDate(Timestamp.valueOf(mapret.get("m19_7").toString()), "yyyy年MM月dd日HH时mm分"));
		...
		List<Map<String, Object>> wlflist=dtzyDao.findallWlfIds(mapret.get("m19_6").toString()+"0");
		String wlfname="";
		int num=1;
		...
		map.put("m10_1", mapret.get("m10_1"));
		...
		if(mapret.get("m19_2")!=null&&!mapret.get("m19_2").toString().equals("")){
			String[] url = mapret.get("m19_2").toString().split("[||]");
			map.put("img2", WordUtil.getImageStr(webAddress + url[0]));
		}else{
			map.put("img2", null);
		}
		
		if(mapret.get("m19_3")!=null&&!mapret.get("m19_3").toString().equals("")){
			String[] url = mapret.get("m19_3").toString().split("[||]");
