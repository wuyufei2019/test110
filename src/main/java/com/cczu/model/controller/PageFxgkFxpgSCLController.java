...
import com.cczu.model.service.FxgkFxpgSclActionService;
import com.cczu.model.service.FxgkFxpgSclService;
 ...
 */
@Controller
@RequestMapping("fxpg/scl")
public class PageFxgkFxpgSCLController extends BaseController {

	@Autowired
	private IBisQyjbxxService bisQyjbxxService;
	@Autowired
	private FxgkFxpgSclService fxgkfxpgservice;
	@Autowired
	private FxgkFxpgSclActionService fxgkfxpgsclactionservice;
	@Autowired
...
/**+++++++++++++++++++++++++++++++++++++风险评估++++++++++++++++++++++++++++++++++++++++++****/
	 ...
	 * @param request
	 */
	@RequiresPermissions("fxpg:scl:view")
	 ...
	 * @param id,model
	 */
	@RequiresPermissions("fxpg:scl:update")
	@RequestMapping(value = "update/{id}", method = RequestMethod.GET)
	public String update(@PathVariable("id") long id, Model model) {
		// 查询选择的风险评估信息
		FXGK_SclRiskAssessment entity = fxgkfxpgservice.findById(id);
	...
	public String create(HttpServletRequest request) {
		...
		FXGK_SclRiskAssessment e= JsonMapper.getInstance().fromJson(entity,FXGK_SclRiskAssessment.class);
		e.setS1(t);
		e.setS2(t);
		e.setS3(0);
		...
		if(id>0){
			...
			List<FXGK_SclRiskAction> l= JSON.parseArray(list, FXGK_SclRiskAction.class);
			for(FXGK_SclRiskAction action: l){
				action.setID1(id);
				fxgkfxpgsclactionservice.addInfo(action);
	...
	@ResponseBody
	public String update(FXGK_SclRiskAssessment entity, Model model) {
	 ...
	 */
	@RequiresPermissions("fxpg:scl:delete")
	...
	public String updateAction(Model model,HttpServletRequest request) {
		...
		model.addAttribute("entity", JsonMapper.getInstance().fromJson(data, FXGK_SclRiskAction.class));
		return "fxgk/fxpg/scl/actionform";
	}
	...
	public String viewAction(@PathVariable("id")long id) {
		List<FXGK_SclRiskAction> list=fxgkfxpgsclactionservice.findAllById1(id);
	...
	public String updateaction(@PathVariable("id")long id,Model model,HttpServletRequest request) {
		FXGK_SclRiskAction entity= fxgkfxpgsclactionservice.findById(id);
		model.addAttribute("entity", entity);
	...
	@ResponseBody
	public String createAct(FXGK_SclRiskAction entity, Model model) {
		String datasuccess = "success";
		fxgkfxpgsclactionservice.addInfo(entity);
		FXGK_SclRiskAssessment assessment=fxgkfxpgservice.findById(entity.getID1());
	...
	@ResponseBody
	public String updateAct(FXGK_SclRiskAction entity, Model model) {
		String datasuccess = "success";
		FXGK_SclRiskAction old = fxgkfxpgsclactionservice.findById(entity.getID());
		...
		fxgkfxpgsclactionservice.updateInfo(entity);
		// 返回结果
		return datasuccess;
	}
	...
	public String deleteAct(@PathVariable("id") long id) {
		String datasuccess = "删除成功";
		fxgkfxpgsclactionservice.deleteInfo(id);
	...
	public void addMajorRisk(FXGK_SclRiskAction action,FXGK_SclRiskAssessment e){
		...
		risk.setName(action.getProject());
	 ...
	 * 
	 */
	@RequiresPermissions("fxpg:scl:export")
	...
	public String exportWord(@PathVariable("id") String ids,HttpServletRequest request, HttpServletResponse response) {
		...
		WordUtil.ireportWord(remap, "fxpg_scl.ftl", filePath, filename, request);
