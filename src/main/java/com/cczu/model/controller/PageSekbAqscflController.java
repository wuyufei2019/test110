 ...
 */
@Controller
@RequestMapping("sekb/aqscfl")
public class PageSekbAqscflController extends BaseController {
	 ...
	 * @param request
	 */
	@RequiresPermissions("sekb:aqscfl:view")
	...
	public Map<String, Object> getData(HttpServletRequest request) {
		...
		map.put("type", "1");//法律
		map.put("btname", request.getParameter("sekb_aqscfl_bt_name"));
	 ...
	 * @param model
	 */
	@RequiresPermissions("sekb:aqscfl:add")
	@RequestMapping(value = "create", method = RequestMethod.GET)
	public String create(Model model) {
		model.addAttribute("action", "create");
		model.addAttribute("usertype",UserUtil.getCurrentShiroUser().getUsertype());
		return "sekb/aqscfl/form";
	}
	 ...
	 * @param id,model
	 */
	@RequiresPermissions("sekb:aqscfl:update")
	 ...
	 */
	@RequiresPermissions("sekb:aqscfl:delete")
	 ...
	 * 
	 */
	@RequiresPermissions("sekb:aqscfl:excel")
	...
	public String getWord(@PathVariable("id") Long id,HttpServletRequest request, HttpServletResponse response) {
		...
		WordUtil.ireportWord(map, "aqfl.ftl", filePath, filename, request);
	...
	@ResponseBody
	public String qrcode(Long id,HttpServletResponse response,HttpServletRequest request) {
		TMESK_LawsEntity ee = sekbAqscfgService.findById(id);
		String text=" ";
		if(!StringUtils.isEmpty(ee.getQrcode()))
			text=ee.getQrcode();
		// 取得输出流        
		String dowmloadPath = request.getSession().getServletContext().getRealPath("/")+"/download";
		String url="/download/";
		try {
			url =url+ QRCode.encode2(text, dowmloadPath, true,"",ee.getM2());
		} catch (Exception e) {
