 ...
 *
 */
@Repository("SBSSGLSbbyTaskFirDao")
public class SBSSGLSbbyTaskFirDao extends BaseDao<SBSSGL_SBBYTASKFIREntity, Long> {

	/**
     * 根据Map分页查找设备一级保养任务list
     * @param id
     * @return
     */
	public List<Map<String, Object>> findPageListByMap(Map<String, Object> mapData) {
		...
		String sql = " SELECT top "+mapData.get("pageSize")+" * FROM ( SELECT ROW_NUMBER() OVER (ORDER BY sbgl.id desc) AS RowNumber, a.*, sbgl.m1 sbbh, sbgl.m2 sbname, sbgl.m3 sbgg, sbgl.m27 sbxh, dept.m1 deptname "
    			   ...
    			   + " WHERE a.s3 = 0 AND task.type = '0' AND task.s3 = 0 AND bis.s3 = 0 AND sbgl.s3 = 0 " 
    			   + content +" ) "
    ...
    public int getPageTotalCountByMap(Map<String, Object> mapData) {
    	...
    	String sql = " SELECT COUNT(*) sum "
    			   ...
    			   + " INNER JOIN sbssgl_sbgl sbgl ON a.sbid = sbgl.id "
    			   + " LEFT JOIN t_user tuser ON task.bzrid = tuser.id "
    			   + " WHERE a.s3 = 0 AND task.type = '0' AND task.s3 = 0 AND bis.s3 = 0 AND sbgl.s3 = 0 " + content;
    ...
    public String content(Map<String, Object> mapData) {
		...
		if(mapData.get("year")!=null&&mapData.get("year")!=""){
			content = content +" AND task.year = '"+mapData.get("year")+"' "; 
		}
		if(mapData.get("month")!=null&&mapData.get("month")!=""){
			content = content +" AND task.month = '"+mapData.get("month")+"' "; 
		}
    ...
    public void deleteInfo(Long id) {
		String sql="UPDATE sbssgl_sbbytaskfir SET s3=1 WHERE id="+id;
		updateBySql(sql);
	}
    ...
    public void deleteInfoByTaskid(Long taskid) {
		String sql="UPDATE sbssgl_sbbytaskfir SET s3=1 WHERE taskid="+taskid;
		updateBySql(sql);
	}
