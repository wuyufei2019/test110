 ...
 *
 */
@Repository("TargetMbzpDao")
public class TargetMbzpDao extends BaseDao<Target_SelfAssessments, Long>{
	...
	public List<Map<String,Object>> dataGrid(Map<String, Object> mapData) {
		...
		String sql =" SELECT top "+mapData.get("pageSize")+" * FROM (SELECT ROW_NUMBER() OVER ("+ordercont+") "
				...
				+ " from target_selfassessments a left join target_distribute b "
    ...
    public int getTotalCount(Map<String, Object> mapData) {
		String content=content(mapData);
		String sql=" SELECT COUNT(a.id) sum from target_selfassessments a left join target_distribute b "
				+ " on a.id2=b.id left join target_basic c on c.id=b.id1 "
				+ " left join bis_enterprise bis on bis.id=a.id1  WHERE a.s3=0 and bis.s3=0 and c.s3=0 and b.s3=0 "+ content;
    ...
    public String content(Map<String, Object> mapData) {
		...
		if(mapData.get("aqtzstartnf")!=null&&mapData.get("aqtzstartnf")!=""){
			content = content +" AND b.m1 >='"+mapData.get("aqtzstartnf")+"' "; 
		}
		if(mapData.get("aqtzfinishnf")!=null&&mapData.get("aqtzfinishnf")!=""){
			content = content +" AND b.m1 <='"+mapData.get("aqtzfinishnf")+"' "; 
		}
    ...
    public void deleteInfo(Long id) {
		String sql=" delete from target_selfassessments  WHERE ID="+id;
		updateBySql(sql);
	}
    //修改
    public void updateInfo(Target_SelfAssessments entity) {
    	String sql=" update target_selfassessments set s2='"+entity.getS2()+"', m2='"+entity.getM2()+"',m4="+entity.getM4()+",m3='"+entity.getM3()+"' WHERE ID="+entity.getID();
    	updateBySql(sql);
    }
    /**
     * 查找
    ...
    public Target_SelfAssessments findInfoById1(Long id) {
    	String sql=" select  * from target_selfassessments WHERE ID1="+id;
    	List<Target_SelfAssessments> list=findBySql(sql, null,Target_SelfAssessments.class);
    	if(list.size()>0)
    		return list.get(0);
    	else
    		return null;
    ...
    public List<Map<String, Object>> findQuartById(Map<String, Object> mapData) {
    	...
    	String sql=" SELECT b.id, a.id zpid,a.m4 result,b.targetval,c.m1 tname from"
    			...
    			+ " LEFT JOIN target_selfassessments a on b.id=a.id2 and(a.quarter ="+mapData.get("quart")+") left join bis_enterprise bis on bis.id=b.id2"
    			+ " where b.s3=0 and c.s3=0 and (a.s3=0 or a. s3 is null)"+content;
    	List<Map<String, Object>> list=findBySql(sql,null,Map.class);
		return  list;
    }
    ...
    public List<Map<String, Object>> getExport(Map<String, Object> mapData) {
    	String content=content(mapData);
		String sql="SELECT a.*,c.m1 targetname,bis.m1 qyname,b.m1 year,td.m1 department,b.targetval,case a.m4 when '1' then '达标' else '未达标' end dbqk,CONVERT(varchar(100), a.m2, 23) pdrq,"
				+ "'第'+a.quarter+'季度' jd from target_selfassessments a left join target_distribute b "
				+ " on a.id2=b.id left join target_basic c on c.id=b.id1 left join bis_enterprise bis on bis.id=a.id1"
				+ " left join t_department td on td.id=b.id3 WHERE bis.S3=0 and a.S3=0 and c.s3=0 and b.s3=0 "+ content 
				+ " ORDER BY bis.m1,b.m1 desc,b.id3,a.quarter,c.id";
		List<Map<String, Object>> list=findBySql(sql, null,Map.class);
		return list;
	}
}
