...
public class AqscFytqDao extends BaseDao<AQSC_ExpenseExtraction, Long>{
	...
	public List<Map<String, Object>> dataGrid2(Map<String, Object> mapData) {
		String content=content(mapData);
		String ordercont=setSortWay(mapData,"","ORDER BY a.m1 desc,a.id desc");
		String sql =" SELECT top "+mapData.get("pageSize")+" * FROM ( SELECT ROW_NUMBER() OVER ("+ordercont+") AS RowNumber,bis.m1 qyname,a.* FROM bis_enterprise bis"
				+ " LEFT JOIN aqsc_expenseextraction a on a.qyid=bis.id where 1=1 "+content+" ) "
				+ " as a WHERE RowNumber > "+mapData.get("pageSize")+"*("+mapData.get("pageNo")+"-1) " ;
		List<Map<String, Object>> list=findBySql(sql, null,Map.class);
	...
	public int getTotalCount2(Map<String, Object> mapData) {
		...
		String sql=" SELECT COUNT(*) sum  FROM bis_enterprise bis "
				+ " left join aqsc_expenseextraction a on bis.id=a.qyid WHERE bis.S3=0 "+ content;
		List<Object> list=findBySql(sql);
		return (int) list.get(0);
	}
}
