 ...
 * @author YZH
 */
@Repository("HjbhInStorageDao")
public class HjbhInStorageDao extends BaseDao<HJBH_InStorage, Long>{
	...
	public List<Map<String,Object>> dataGrid(Map<String, Object> mapData) {
		...
		String sql =" SELECT top "+mapData.get("pageSize")+" * FROM ( SELECT ROW_NUMBER() OVER ("+ordercont+") AS "
				+ "RowNumber,a.* ,b.name FROM HJBH_InStorage a left join hjbh_wxgl b on a.id1=b.id "
    ...
    public int getTotalCount(Map<String, Object> mapData) {
		String content=content(mapData);
		String sql="SELECT COUNT(a.id) sum FROM HJBH_InStorage a left join hjbh_wxgl b on a.id1=b.id "
    ...
    public void deleteInfo(Long id) {
		String sql=" UPDATE HJBH_InStorage SET S3=1 WHERE ID="+id;
		updateBySql(sql);
	}
