...
/**
 * 在线监控预警-压力传感器数据controller
 * @author jason
 * @date 2017年6月23日
 */
@Controller
@RequestMapping("zxjkyj/ylcgq")
public class PageMonitorPressureController extends BaseController {

	@Autowired
	private IMonitorPressureService monitorPressureService;
	@Autowired
	...
	public String index(Model model) {
		...
		return "zxjkyj/ssjc/ylcgq/index";
	}
	...
	public Map<String, Object> getData(HttpServletRequest request) {
		...
		map.put("qyname", request.getParameter("ssjc_ylcgq_qy_name"));
		map.putAll(getAuthorityMap());
		return monitorPressureService.dataGrid(map);
	}
	...
	public String selectQyJson(Model model,HttpServletRequest request) {
		return monitorPressureService.qyListJson(getAuthorityMap());
	}
	...
	public Map<String, Object> list(HttpServletRequest request) {
		...
		List<Map<String,Object>> list=monitorPressureService.findMapList(mapdata);
	...
	public void export(String ids, HttpServletResponse response) {
		...
		monitorPressureService.exportExcel(response, map);
	...
	public String view(@PathVariable("id") Long id, Model model) {
		List<Map<String, Object>> cllist=monitorPressureService.findInfoByQyid(id);
		BIS_EnterpriseEntity ent= qyjbxxService.findInfoById(id);
		model.addAttribute("cllist", cllist);
		model.addAttribute("qiye", ent);
		return "zxjkyj/ssjc/ylcgq/view";
	} 
}
