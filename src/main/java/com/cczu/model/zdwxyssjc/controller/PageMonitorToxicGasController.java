...
/**
 * 在线监控预警-有毒气体传感器数据controller
 * @author jason
 * @date 2017年6月23日
 */
@Controller
@RequestMapping("zxjkyj/ydqtcgq")
public class PageMonitorToxicGasController extends BaseController {
	...
	public String index(Model model) {
		...
		return "zxjkyj/ssjc/ydqtcgq/index";
	}
	...
	public Map<String, Object> getData(HttpServletRequest request) {
		...
		map.put("qyname", request.getParameter("ssjc_ydqtcgq_qy_name"));
		map.putAll(getAuthorityMap());
		return monitorToxicGasService.dataGrid(map);
	}
	...
	public String selectQyJson(Model model,HttpServletRequest request) {
		return monitorToxicGasService.qyListJson(getAuthorityMap());
	}
	...
	public Map<String, Object> list(HttpServletRequest request) {
		...
		List<Map<String,Object>> list=monitorToxicGasService.findMapList(mapdata);
	...
	public void export(String ids, HttpServletResponse response) {
		...
		monitorToxicGasService.exportExcel(response, map);
	...
	public String view(@PathVariable("id") Long id, Model model) {
		List<Map<String, Object>> cllist=monitorToxicGasService.findInfoByQyid(id);
		BIS_EnterpriseEntity ent= qyjbxxService.findInfoById(id);
		model.addAttribute("cllist", cllist);
		model.addAttribute("qiye", ent);
		return "zxjkyj/ssjc/ydqtcgq/view";
	}
}
