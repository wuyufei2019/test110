 ...
 * 现场供气信息
 */
@Repository("bisFieldSupplyDao")
public class BisFieldSupplyDaoImpl extends BaseDao<BIS_FieldSupplyEntity,Long> {

	public List<BIS_FieldSupplyEntity> findAll(long qyid) {

		String sql ="SELECT * FROM bis_fieldsupply WHERE s3=0 AND ID1="+qyid;
		List<BIS_FieldSupplyEntity> list=findBySql(sql, null, BIS_FieldSupplyEntity.class);
		return list;
	}

	public List<BIS_FieldSupplyEntity> dataGrid(Map<String, Object> mapData) {
		String content=content(mapData);
		String ordercont=setSortWay(mapData,"","ORDER BY a.id desc");
		String sql =" SELECT top "+mapData.get("pageSize")+" * FROM ( SELECT ROW_NUMBER() OVER ("+ordercont+") AS RowNumber,* FROM bis_fieldsupply a WHERE S3=0 "+content+" ) "
	...
	public int getTotalCount(Map<String, Object> mapData) {
		String content=content(mapData);
		String sql=" SELECT COUNT(*) sum  FROM bis_fieldsupply a WHERE s3=0  AND 1=1"+ content;
	...
	public BIS_FieldSupplyEntity findById(Long id) {
		String sql ="SELECT * FROM bis_fieldsupply WHERE s3=0 AND ID="+id;
		List<BIS_FieldSupplyEntity> list=findBySql(sql, null,BIS_FieldSupplyEntity.class);
		if(list.size()>0){
			return list.get(0);
		}
	...
	public void deleteInfo(Long id) {
		String sql=" UPDATE bis_fieldsupply SET S3=1 WHERE ID="+id;
		updateBySql(sql);
	}
	...
	public List<Map<String, Object>> getExport(Map<String, Object> mapData) {
		...
		String content=content(mapData);
		String sql=" SELECT a.m1 ,Round(a.m2,2) m2,Round(a.m3,2) m3,(case a.m4 when '0' then '自建' when '1' then '租用' end) m4,a.m5,a.m6_1,a.m7_1,CONVERT(varchar(100), a.m8_1, 23) m8_1,a.m6_2,a.m7_2,CONVERT(varchar(100), a.m8_2, 23) m8_2,a.m6_3,a.m7_3,CONVERT(varchar(100), a.m8_3, 23) m8_3,b.m1 qyname "
				+ " FROM bis_fieldsupply a left join bis_enterprise b on b.id=a.id1 where a.S3=0 and b.S3=0 "+ content +" order by b.id";
	...
	public List<Map<String, Object>> dataGridAJ(Map<String, Object> map) {
		...
		String sql =" SELECT top "+map.get("pageSize")+" * FROM ( SELECT ROW_NUMBER() OVER ("+ordercont+") AS RowNumber, "
				+ " b.m1 as qyname,a.* FROM bis_fieldsupply a "
	...
	public int getTotalCountAJ(Map<String, Object> map) {
		String content=content(map);
		String sql=" SELECT COUNT(*) sum  FROM bis_fieldsupply a left join bis_enterprise b on a.id1=b.id  WHERE a.s3=0 and b.s3=0 "+ content;
		List<Object> list=findBySql(sql);
		return (int) list.get(0);
	}
