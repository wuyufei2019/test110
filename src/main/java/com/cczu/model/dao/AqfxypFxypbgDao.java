...
@Repository("AqfxypFxypbgDao")
public class AqfxypFxypbgDao extends BaseDao<AQFXYP_RiskReports, Long> {
    ...
    public int getTotalCount(Map<String, Object> mapData) {
        String sql = " SELECT COUNT(1) sum  FROM aqfxyp_riskreports a "
    ...
    public void deleteInfo(Long id) {
        String sql = " UPDATE aqfxyp_riskreports SET S3=1 WHERE ID=" + id;
        updateBySql(sql);
    }

    public Map<String,Object> export(long id) {
        String sql = " SELECT a.*,bis.m1 qyname from aqfxyp_riskreports a left join bis_enterprise bis ON bis.id = a.ID1 "
    ...
    public Map<String,Object> findLatestInfo(Map<String, Object> map) {
        String content = content(map);
        String sql = " SELECT a.* from aqfxyp_riskreports a "
                + " WHERE a.id = (SELECT MAX(a.id) FROM aqfxyp_riskreports a WHERE a.s3 = 0 " + content + ") ";
        List<Map<String,Object>> list = findBySql(sql, null, Map.class);
        if(list.size()>0)
            return list.get(0);
        else
