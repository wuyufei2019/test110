...
var d;
$(function(){
	dg=$('#zdgl_gcps_dg').datagrid({    
	method: "post",
    url:ctx+'/zdgl/aqps/list2?qyid='+qyid,
    fit : true,
	fitColumns : true,
	border : false,
	idField : 'id',
	...
	checkOnSelect:false,
	selectOnCheck:false,
    toolbar:'#zdgl_gcps_tb'
	});
});

//审核
function addsh(id){
	openDialog("审核规程评审信息",ctx+"/zdgl/aqps/update2/sh/"+id,"800px", "400px","");
}

//批准
function addsp(id){
	openDialog("批准规程评审信息",ctx+"/zdgl/aqps/update2/pz/"+id,"800px", "400px","");
}
...
function view(){
	...
	openDialogView("查看安全操作规程评审信息",ctx+"/zdgl/aqps/view2/"+row.id,"800px", "400px","");
}
...
//弹窗增加
function add() {
	openDialog("添加安全操作规程评审信息",ctx+"/zdgl/aqps/create2/","800px", "400px","");
}
...
function upd(){
	...
	if(row.m16 == '1'){
		...
		openDialog("修改安全操作规程评审信息",ctx+"/zdgl/aqps/update2/xg/"+row.id,"800px", "400px","");
	}else if(row.m13 == '1'){
