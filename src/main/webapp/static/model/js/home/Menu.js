...
Menu.prototype._show = function($this){
    ...
    if (!$this.hasClass("threenav-li")){
        this.nav.find('li').removeClass('current');
        //cap
        if(this.nav.find('li[data-id="'+ id +'"]').hasClass('threenav-li')){
        	this.nav.find('li[data-id="'+ id +'"]').addClass('currentcon').parents('.nav-li').siblings().find(".subnav").hide().end().end().addClass('current').find(".subnav").show();
        	this.nav.find('li[data-id="'+ id +'"]').parents('.nav-li').siblings().find(".subnav").find(".subnav-li").removeClass("current").find(".three-nav").hide().find(".threenav-li").removeClass("currentcon");
        	this.nav.find('li[data-id="'+ id +'"]').parents('ul').show().parents('.subnav-li').addClass('current');
        	this.nav.find('li[data-id="'+ id +'"]').parents('.subnav-li').siblings("").find(".three-nav").hide().find(".threenav-li").removeClass("currentcon");
        }else{
        ...
        this.nav.find('li[data-id="'+ id +'"]').siblings().find("ul").hide();
    }
...
Menu.prototype._bindEvent = function(){
    ...
    this.nav.on('click', 'li:not(".threenav-li")', function(){
        ...
        $(this).siblings().find(".subnav").find(".subnav-li.current").removeClass("current").find(".three-nav").hide().find(".threenav-li.currentcon").removeClass("currentcon"); 
    });


    this.nav.on('click', '.subnav-li', function(){

        if($(this).hasClass('current') || $(this).hasClass('threenav-li')){
            // 点击二级菜单时隐藏三级菜单列表 0710
            $(this).removeClass("current").find("ul").first().hide();
            return false;
        }
        $(this).addClass('current').siblings().removeClass("current").find(".three-nav").hide().find(".threenav-li.currentcon").removeClass("currentcon").end().end().end().find(".three-nav").show();
    });

    // 一级菜单展开后点击隐藏0710
    this.nav.on('click', '.nav-li.current', function(){
        $(this).removeClass("current");
        return false;
    });
    this.nav.on('click', '.nav-li', function(){
    ...
    // 这里会默认打开第一个三级菜单0710
    this.nav.on('click', '.subnav li:not(".threenav-li")', function(){
        if ($(this).find(".three-nav").length == 0){
            self._select($(this));
        }
        return false;
    });

    this.nav.on('click', '.three-nav li', function(){
        $(".threenav-li").removeClass("currentcon");
