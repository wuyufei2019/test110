...
function addReCheck(id){
	top.layer.confirm('添加复查记录后初查记录将不能修改！', {
		...
		openDialog("添加复查记录", ctx + "/dsffw/jcjl/addReCheck/"+id, "800px", "400px", "");
	});
}

//修改复查信息
function updateReCheck(id){
	openDialog("修改复查记录", ctx + "/dsffw/jcjl/updateCheck/"+id, "800px", "400px", "");
}
...
function addNr() {
	layer.open({
	    ...
	    title: '添加问题',
        maxmin: false, 
	    content: ctx + "/dsffw/jcjl/choosenr" ,
	    btn: ['确定', '关闭'],
