...
var d;
$(function(){
	dg=$('#fxpg_jha_dg').datagrid({    
	method: "post",
    url:ctx+'/fxpg/jha/list', 
    fit : true,
	fitColumns : true,
	border : false,
	idField : 'id',
...
function viewaction(id) {
	...
	layer.open({
	    ...
	    success: function(layero, index){
	    	    d =$('#action').datagrid({
	    	    	url :ctx+"/fxpg/jha/actionlist/" + id,
	    	    	fitColumns : true,
					border : true,
					striped : true,
					rownumbers : true,
...
//弹窗增加
function add(u) {
	openDialog("添加风险评估信息",ctx+"/fxpg/jha/create/","90%", "90%","");
}
function addAction() {
	openDialog("添加风险活动信息",ctx+"/fxpg/jha/actioncreate/","700px", "400px","");
}
function updaction(id) {
	openDialog("修改风险活动信息",ctx+"/fxpg/jha/updateaction/"+id ,"700px", "400px","");
}
function deleterow(id) {
	top.layer.confirm('删除后无法恢复您确定要删除？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/fxpg/jha/deleteact/"+id,
...
function upd(){
	...
	openDialog("修改风险评估信息",ctx+"/fxpg/jha/update/"+row.id,"800px", "400px","");
...
function view(){
	...
	openDialogView("查看风险评估信息",ctx+"/fxpg/jha/view/"+row.id,"90%", "90%","");
...
function exin(id){
    openImportDialog(ctx+'/fxpg/jha/exinjump',ctx+'/fxpg/jha/exin/'+id,ctx+'/static/templates/jha风险活动导入模板.xls')
}
...
function fileexportword(){
	...
	$.ajax({
		url:ctx+"/fxpg/jha/exportword/"+ids,
		type:"POST",
		success:function(data){
