...
$(function(){
	...
	method: "post",
    url:ctx+'/ztzyaqgl/sxzy/list?spzt='+spzt,
    fit : true,
	fitColumns : true,
	border : false,
	idField : 'id',
    ...
    columns:[[    
              ...
              {field:'m13',title:'受限空间作业级别',width:50,align:'center'},
              ...
              {field:'zt',title:'状态',width:50,align:'center',
            	  formatter : function(value, row, index){
            		  ...
            		  if(value==0){
            			  ...
            			  if(role8==1){
            				  a="<a style='margin:2px' class='btn btn-success btn-xs' onclick='fx("+row.id+")'>分析</a><br/>";
            			  }else{
							  a="待分析<br/>";
						  }
            			  if(role3==1||role4==1){
            			  ...
            			  if(role0==1||role1==1||role9==1){
            				  a+="<a style='margin:2px' class='btn btn-success btn-xs' onclick='dwqz("+row.id+")'>作业单位完工签字</a>";
            			  }
            			  if(role3==1||role4==1){
            				  a+="<a style='margin:2px' class='btn btn-success btn-xs' onclick='cancel("+row.id+")'>作废</a>";
            			  }
            			  ...
            			  a= '已完工';
						  if(row.m13=='特殊'&&isToday(row.m21_7)&&delay==1){
							  a+=" <a style='margin:2px' class='btn btn-success btn-xs' onclick='aaa("+row.id+")'>延期</a>";
						  }
						  return a;
            		  }else if(value==(-1)){
            			  if(row.m21_1==userid){
            		  ...
            		  //判断是否过期
					  if(row.m21_7!=null&&row.m21_7!='') {
						  var hour = compare(row.m21_7);
						  if (row.zt != 10 && row.m13 != '特殊' && hour > 7 * 24) {
							  a = "已过期";
						  }
						  if (row.zt != 10 && row.m13 == '特殊' && hour > 8) {
							  a = "已过期";
						  }
					  }
            		  return a;
...
//添加受限空间作业信息
function add(u) {
	openDialog("申请受限空间作业",ctx+"/ztzyaqgl/sxzy/create/","900px", "90%","");
}

//编制安全措施
function addbzcs(id) {
	openDialog("编制安全措施",ctx+"/ztzyaqgl/sxzy/aqcsindex?id1="+id,"800px", "600px","");
}

//延期
function aaa(id) {
	openDialog("作业延期",ctx+"/ztzyaqgl/sxzy/delay?sxid="+id,"800px", "380px","");
}

//补充安全措施
function updaqcs(id) {
	openDialog("编制安全措施",ctx+"/ztzyaqgl/sxzy/updaqcsindex?id1="+id,"800px", "600px","");
}

//作业单位确认
function zydwqr(id) {
	openDialog("作业单位确认",ctx+"/ztzyaqgl/sxzy/zydwqr/"+id,"800px", "300px","");
}

//安技员审批
function ajysp(id) {
	openDialog("安技员审批",ctx+"/ztzyaqgl/sxzy/nbsp/"+id,"800px", "300px","");
}

//分厂安全分管领导审批
function aqkzsp(id) {
	openDialog("分厂安全分管领导审批",ctx+"/ztzyaqgl/sxzy/nbsp/"+id,"800px", "300px","");
}

//部门一把手审批
function ybssp(id) {
	openDialog("部门一把手审批",ctx+"/ztzyaqgl/sxzy/nbsp/"+id,"800px", "300px","");
}

//安全处分管人员审批
function aqrysp(id) {
	openDialog("安全处分管人员审批",ctx+"/ztzyaqgl/sxzy/nbsp/"+id,"800px", "300px","");
}

//安全处分管领导审批
function aqldsp(id) {
	openDialog("安全处分管领导审批",ctx+"/ztzyaqgl/sxzy/nbsp/"+id,"800px", "300px","");
}

//保卫部审批
function bwbsp(id) {
	openDialog("保卫部审批",ctx+"/ztzyaqgl/sxzy/nbsp/"+id,"800px", "300px","");
}

//公司分管领导审批
function gsldsp(id) {
	openDialog("公司分管领导审批",ctx+"/ztzyaqgl/sxzy/nbsp/"+id,"800px", "300px","");
}

//能源中心审批
function nyzxsp(id) {
	openDialog("能源中心审批",ctx+"/ztzyaqgl/sxzy/nbsp/"+id,"800px", "300px","");
}

//确认安全措施
function qrcs(id) {
	openDialog("确认安全措施",ctx+"/ztzyaqgl/sxzy/qraqcsindex?id1="+id+"&type=1","800px", "400px","");
}

//分析
function fx(id){
	openDialog("受限空间作业分析",ctx+"/ztzyaqgl/sxzy/fx/"+id,"70%", "400px","");
}

//作业完工签字
function dwqz(id) {
	openDialog("作业单位完工签字",ctx+"/ztzyaqgl/sxzy/wgqzindex?id1="+id,"800px", "400px","");
}

//分厂完工签字
function fcqz(id) {
	openDialog("分厂完工签字",ctx+"/ztzyaqgl/sxzy/wgqzindex?id1="+id,"800px", "400px","");
}
...
function viewzt(){
	...
	openDialogView("受限空间作业证状态页面",ctx+"/ztzyaqgl/sxzy/viewzt/"+row.id,"800px", "400px","");
}
...
function del(){
	...
	top.layer.confirm('删除后无法恢复您确定要删除？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/ztzyaqgl/sxzy/delete/"+ids,
...
//弹窗修改
function upd(id){
	openDialog("修改受限空间作业信息",ctx+"/ztzyaqgl/sxzy/update/"+id,"90%", "85%","");
...
function revoke(id){
	top.layer.confirm('确认撤回？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/ztzyaqgl/sxzy/revoke/"+id,
			success: function(data){
				layer.alert(data, {offset: 'rb',shade: 0,time: 2000}); 
				top.layer.close(index);
				dg.datagrid('reload');
...
function cancel(id){
	top.layer.confirm('确认作废该申请？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/ztzyaqgl/sxzy/cancel/"+id,
			success: function(data){
				layer.alert(data, {offset: 'rb',shade: 0,time: 2000}); 
				top.layer.close(index);
				dg.datagrid('reload');
...
function gb(){
	...
	if(row==null) {
		...
		top.layer.confirm('确认关闭？', {icon: 3, title:'提示'}, function(index){
			$.ajax({
				type:'get',
				url:ctx+"/ztzyaqgl/sxzy/gb/"+row.id,
				success: function(data){
					layer.alert(data, {offset: 'rb',shade: 0,time: 2000}); 
					top.layer.close(index);
					dg.datagrid('reload');
...
function view(){
	...
	openDialogView("查看受限空间作业信息",ctx+"/ztzyaqgl/sxzy/view/"+row.id,"900px", "100%","");
...
function fileexport(){
	...
	layer.open({
        ...
        maxmin: true, 
        shift: 1,
	    content: ctx+'/ztzyaqgl/sxzy/colindex',
		btn: ['导出'],
...
function fileexportword(){
	...
	$.ajax({
		url:ctx+"/ztzyaqgl/sxzy/exportword/"+row.id,
		type:"POST",
		success:function(data){
