...
var userid;//用户id
$(function(){
	dg=$('#zyaqgl_gczy_dg').datagrid({    
	method: "post",
    url:ctx+'/ztzyaqgl/gczy/list?spzt='+spzt,
    fit : true,
	fitColumns : true,
	border : false,
	idField : 'id',
    ...
    columns:[[    
              ...
              {field:'m10',title:'高处作业级别',width:50,align:'center',
              ...
              {field:'m8',title:'作业单位',width:80,align:'center'},
              {field:'m11',title:'监护人',width:50,align:'center'},
              ...
              {field:'zt',title:'状态',width:50,align:'center',
            	  formatter : function(value, row, index){
            		  ...
            		  if(value==0){
            			  ...
            			  if(role0==1||role1==1||row.m21_1==userid){
					  ...
					  if(row.m21_7!=null&&row.m21_7!='') {
						  ...
						  if (row.m10 != '四级' && hour > 7 * 24 && row.zt != 11) {
							  a = "已过期";
						  }
						  if (row.m10 == '四级' && hour > 24 && row.zt != 11) {
							  a = "已过期";
						  }
					  }
            		  return a;
...
//添加高处作业信息
function add(u) {
	openDialog("申请高处作业",ctx+"/ztzyaqgl/gczy/create/","900px", "90%","");
}

//编制安全措施
function addbzcs(id) {
	openDialog("编制安全措施",ctx+"/ztzyaqgl/gczy/aqcsindex?id1="+id,"800px", "600px","");
}

//补充安全措施
function updaqcs(id) {
	openDialog("编制安全措施",ctx+"/ztzyaqgl/gczy/updaqcsindex?id1="+id,"800px", "600px","");
}

//作业单位确认
function zydwqr(id) {
	openDialog("作业单位确认",ctx+"/ztzyaqgl/gczy/zydwqr/"+id,"800px", "300px","");
}

//安技员审批
function ajysp(id) {
	openDialog("安技员审批",ctx+"/ztzyaqgl/gczy/nbsp/"+id,"800px", "300px","");
}

//分厂安全分管领导审批
function aqkzsp(id) {
	openDialog("分厂安全分管领导审批",ctx+"/ztzyaqgl/gczy/nbsp/"+id,"800px", "300px","");
}

//部门一把手审批
function ybssp(id) {
	openDialog("部门一把手审批",ctx+"/ztzyaqgl/gczy/nbsp/"+id,"800px", "300px","");
}

//安全处分管人员审批
function aqrysp(id) {
	openDialog("安全处分管人员审批",ctx+"/ztzyaqgl/gczy/nbsp/"+id,"800px", "300px","");
}

//安全处分管领导审批
function aqldsp(id) {
	openDialog("安全处分管领导审批",ctx+"/ztzyaqgl/gczy/nbsp/"+id,"800px", "300px","");
}

//公司分管领导审批
function gsldsp(id) {
	openDialog("公司分管领导审批",ctx+"/ztzyaqgl/gczy/nbsp/"+id,"800px", "300px","");
}

//确认安全措施
function qrcs(id) {
	openDialog("确认安全措施",ctx+"/ztzyaqgl/gczy/qraqcsindex?id1="+id+"&type=1","800px", "400px","");
}

//作业完工签字
function dwqz(id) {
	openDialog("作业单位完工签字",ctx+"/ztzyaqgl/gczy/wgqzindex?id1="+id,"800px", "400px","");
}

//分厂完工签字
function fcqz(id) {
	openDialog("分厂完工签字",ctx+"/ztzyaqgl/gczy/wgqzindex?id1="+id,"800px", "400px","");
}
...
function viewzt(){
	...
	openDialogView("高处作业证状态页面",ctx+"/ztzyaqgl/gczy/viewzt/"+row.id,"800px", "400px","");
}
...
function del(){
	...
	top.layer.confirm('删除后无法恢复您确定要删除？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/ztzyaqgl/gczy/delete/"+ids,
...
//弹窗修改
function upd(id){
	openDialog("修改高处作业信息",ctx+"/ztzyaqgl/gczy/update/"+id,"90%", "85%","");
...
function revoke(id){
	top.layer.confirm('确认撤回？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/ztzyaqgl/gczy/revoke/"+id,
			success: function(data){
				layer.alert(data, {offset: 'rb',shade: 0,time: 2000}); 
				top.layer.close(index);
				dg.datagrid('reload');
...
function cancel(id){
	top.layer.confirm('确认作废该申请？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/ztzyaqgl/gczy/cancel/"+id,
			success: function(data){
				layer.alert(data, {offset: 'rb',shade: 0,time: 2000}); 
				top.layer.close(index);
				dg.datagrid('reload');
...
function gb(){
	...
	if(row==null) {
		...
		layer.msg("该高处作业证已关闭！",{time: 1000});
		return;
	}else{
		top.layer.confirm('确认关闭？', {icon: 3, title:'提示'}, function(index){
			$.ajax({
				type:'get',
				url:ctx+"/ztzyaqgl/gczy/gb/"+row.id,
				success: function(data){
					layer.alert(data, {offset: 'rb',shade: 0,time: 2000}); 
					top.layer.close(index);
					dg.datagrid('reload');
...
function view(){
	...
	openDialogView("查看高处作业信息",ctx+"/ztzyaqgl/gczy/view/"+row.id,"900px", "100%","");
...
function fileexport(){
	...
	layer.open({
        ...
        maxmin: true, 
        shift: 1,
	    content: ctx+'/ztzyaqgl/gczy/colindex',
		btn: ['导出'],
...
function fileexportword(){
	...
	$.ajax({
		url:ctx+"/ztzyaqgl/gczy/exportword/"+row.id,
		type:"POST",
		success:function(data){
