...
var userid;//用户id
$(function(){
	dg=$('#zyaqgl_dtzy_dg').datagrid({    
	method: "post",
    url:ctx+'/ztzyaqgl/dtzy/list?spzt='+spzt,
    fit : true,
	fitColumns : true,
	border : false,
	idField : 'id',
    ...
    columns:[[    
              ...
              {field:'m7',title:'作业地点',width:80,align:'center'},
              {field:'m8',title:'作业单位',width:50,align:'center'},
              {field:'m4',title:'安全监护人',width:50,align:'center'},
              {field:'s1',title:'作业时间',width:100,align:'center',
          	    formatter : function(value, row, index) {
                	if(value!==null&&value!='') {
                		var datetime1=new Date(row.m5);
                		var datetime2=new Date(row.m6);
              ...
              {field:'zt',title:'状态',width:50,align:'center',
            	  formatter : function(value, row, index){
            		  ...
            		  //判断是否过期
					  if(row.m19_7!=null&&row.m19_7!='') {
						  var hour = compare(row.m19_7);
...
//添加动土作业信息
function add(u) {
	openDialog("申请动土作业",ctx+"/ztzyaqgl/dtzy/create/","900px", "90%","");
}

//编制安全措施
function addbzcs(id) {
	openDialog("编制安全措施",ctx+"/ztzyaqgl/dtzy/aqcsindex?id1="+id,"800px", "600px","");
}

//补充安全措施
function updaqcs(id) {
	openDialog("编制安全措施",ctx+"/ztzyaqgl/dtzy/updaqcsindex?id1="+id,"800px", "600px","");
}

//作业单位确认
function zydwqr(id) {
	openDialog("作业单位确认",ctx+"/ztzyaqgl/dtzy/zydwqr/"+id,"800px", "300px","");
}

//能源中心审批
function nyzxsp(id) {
	openDialog("能源中心审批",ctx+"/ztzyaqgl/dtzy/nbsp/"+id,"800px", "300px","");
}

//分厂审批
function fcsp(id) {
	openDialog("分厂审批",ctx+"/ztzyaqgl/dtzy/nbsp/"+id,"800px", "300px","");
}

//设备处审批
function sbcsp(id) {
	openDialog("设备处审批",ctx+"/ztzyaqgl/dtzy/nbsp/"+id,"800px", "300px","");
}

//保卫处审批
function bwcsp(id) {
	openDialog("保卫处审批",ctx+"/ztzyaqgl/dtzy/nbsp/"+id,"800px", "300px","");
}

//安全处审批
function aqcsp(id) {
	openDialog("安全处审批",ctx+"/ztzyaqgl/dtzy/nbsp/"+id,"800px", "300px","");
}

//确认安全措施
function qrcs(id) {
	openDialog("确认安全措施",ctx+"/ztzyaqgl/dtzy/qraqcsindex?id1="+id+"&type=1","800px", "400px","");
}

//作业完工签字
function dwqz(id) {
	openDialog("作业单位完工签字",ctx+"/ztzyaqgl/dtzy/wgqzindex?id1="+id,"800px", "400px","");
}

//分厂完工签字
function fcqz(id) {
	openDialog("分厂完工签字",ctx+"/ztzyaqgl/dtzy/wgqzindex?id1="+id,"800px", "400px","");
}

//能源中心完工签字
function nyzxqz(id) {
	openDialog("能源中心完工签字",ctx+"/ztzyaqgl/dtzy/wgqzindex?id1="+id,"800px", "400px","");
}
...
function viewzt(){
	...
	openDialogView("动土作业证状态页面",ctx+"/ztzyaqgl/dtzy/viewzt/"+row.id,"800px", "400px","");
}
...
function del(){
	...
	top.layer.confirm('删除后无法恢复您确定要删除？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/ztzyaqgl/dtzy/delete/"+ids,
...
//弹窗修改
function upd(id){
	openDialog("修改动土作业信息",ctx+"/ztzyaqgl/dtzy/update/"+id,"90%", "85%","");
...
function revoke(id){
	top.layer.confirm('确认撤回？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/ztzyaqgl/dtzy/revoke/"+id,
			success: function(data){
				layer.alert(data, {offset: 'rb',shade: 0,time: 2000}); 
				top.layer.close(index);
				dg.datagrid('reload');
...
function cancel(id){
	top.layer.confirm('确认作废该申请？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/ztzyaqgl/dtzy/cancel/"+id,
			success: function(data){
				layer.alert(data, {offset: 'rb',shade: 0,time: 2000}); 
				top.layer.close(index);
				dg.datagrid('reload');
...
function gb(){
	...
	if(row==null) {
		...
		layer.msg("该动土作业证已关闭！",{time: 1000});
		return;
	}else{
		top.layer.confirm('确认关闭？', {icon: 3, title:'提示'}, function(index){
			$.ajax({
				type:'get',
				url:ctx+"/ztzyaqgl/dtzy/gb/"+row.id,
				success: function(data){
					layer.alert(data, {offset: 'rb',shade: 0,time: 2000}); 
					top.layer.close(index);
					dg.datagrid('reload');
...
function view(){
	...
	openDialogView("查看动土作业信息",ctx+"/ztzyaqgl/dtzy/view/"+row.id,"900px", "100%","");
...
function fileexport(){
	...
	window.exdata=[
			   		...
			   		{colval:'m3', coltext:'动土作业级别'},
			   		{colval:'m4', coltext:'动土方式'},
			   		{colval:'m5', coltext:'动土地点'},
			   		{colval:'m6', coltext:'动土时间起'},
			   		{colval:'m7', coltext:'动土时间止'},
			   		{colval:'m8', coltext:'动土作业负责人'},
			   		{colval:'m9', coltext:'动土人'},
			   		...
			   		{colval:'dhcsr', coltext:'动土初审人'},
			   		{colval:'aqjyr', coltext:'实施安全教育人'},
			   		{colval:'m19_1', coltext:'申请单位意见'},
			   		...
			   		{colval:'m24_1', coltext:'动土审批人意见'},
			   		{colval:'dhspr', coltext:'动土审批人'},
	...
	layer.open({
        ...
        maxmin: true, 
        shift: 1,
	    content: ctx+'/ztzyaqgl/dtzy/colindex',
		btn: ['导出'],
...
function fileexportword(){
	...
	$.ajax({
		url:ctx+"/ztzyaqgl/dtzy/exportword/"+row.id,
		type:"POST",
		success:function(data){
