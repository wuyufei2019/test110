...
$(function(){
	if(type==0){
		...
		method: "post",
	    url:ctx+'/ztzyaqgl/dtzy/aqcslist', 
	    fit : true,
		fitColumns : true,
		border : false,
		idField : 'id',
		...
		dg=$('#zyaqgl_dtzy_dg').datagrid({    
		method: "post",
	    url:ctx+'/ztzyaqgl/dtzy/aqcslist?type='+type+'&id1='+id1, 
	    fit : true,
		fitColumns : true,
		border : false,
		idField : 'id',
...
function endEditing(){
	...
	if ($('#zyaqgl_dtzy_dg').datagrid('validateRow', editIndex)){
		$('#zyaqgl_dtzy_dg').datagrid('endEdit', editIndex);
		editIndex = undefined;
		return true;
	} else {
		return false;
...
function onClickCell(index, field){
	if (endEditing()){
		$('#zyaqgl_dtzy_dg').datagrid('selectRow', index)
...
function openwlflist() {
	layer.open({
	    ...
	    title: '选择外来方',
        maxmin: false, 
	    content: ctx + "/ztzyaqgl/dtzy/choose" ,
	    btn: ['确定', '关闭'],
