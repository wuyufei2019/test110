...
$(function(){
	if(type==0){
		...
		method: "post",
	    url:ctx+'/ztzyaqgl/gczy/aqcslist', 
	    fit : true,
		fitColumns : true,
		border : false,
		idField : 'id',
		...
		dg=$('#zyaqgl_gczy_dg').datagrid({    
		method: "post",
	    url:ctx+'/ztzyaqgl/gczy/aqcslist?type='+type+'&id1='+id1, 
	    fit : true,
		fitColumns : true,
		border : false,
		idField : 'id',
...
function endEditing(){
	...
	if ($('#zyaqgl_gczy_dg').datagrid('validateRow', editIndex)){
...
function onClickCell(index, field){
	if (endEditing()){
		$('#zyaqgl_gczy_dg').datagrid('selectRow', index)
...
function openwlflist() {
	layer.open({
	    ...
	    title: '选择外来方',
        maxmin: false, 
	    content: ctx + "/ztzyaqgl/gczy/choose" ,
	    btn: ['确定', '关闭'],
