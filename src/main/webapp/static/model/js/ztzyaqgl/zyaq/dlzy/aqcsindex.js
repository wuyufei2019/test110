...
$(function(){
	if(type==0){
		...
		method: "post",
	    url:ctx+'/ztzyaqgl/dlzy/aqcslist', 
	    fit : true,
		fitColumns : true,
		border : false,
		idField : 'id',
		...
		dg=$('#zyaqgl_dlzy_dg').datagrid({    
		method: "post",
	    url:ctx+'/ztzyaqgl/dlzy/aqcslist?type='+type+'&id1='+id1, 
	    fit : true,
		fitColumns : true,
		border : false,
		idField : 'id',
...
function endEditing(){
	...
	if ($('#zyaqgl_dlzy_dg').datagrid('validateRow', editIndex)){
...
function onClickCell(index, field){
	if (endEditing()){
		$('#zyaqgl_dlzy_dg').datagrid('selectRow', index)
...
function openwlflist() {
	layer.open({
	    ...
	    title: '选择外来方',
        maxmin: false, 
	    content: ctx + "/ztzyaqgl/dlzy/choose" ,
	    btn: ['确定', '关闭'],
