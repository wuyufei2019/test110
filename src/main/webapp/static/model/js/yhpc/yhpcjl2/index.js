...
$(function(){
    ...
    columns:[[    
    			{field : 'id',title : 'id',checkbox : true,width : 50,align : 'center'},
      			{field : 'checkpointname',title : '巡检点名称',sortable : false,width : 50,align : 'center'},
      			{field : 'checkcontent',title : '隐患内容',sortable : false,width : 90,align : 'center'},
    			{field : 'findperson',title : '隐患发现人',sortable : false,width : 30,align : 'center'},
    			...
    			{field : 'handlepersons',title : '指定整改人',sortable : false,width : 40,align : 'center'},
    			{field : 'dangerphoto',title : '隐患附件',sortable : false,width : 40,align : 'center',
    				formatter : function(value, row, index) {
        				var content="";
                      	if(value!=null&&value!='') {
    			...
    			{field : 'reformpersons',title : '整改人',sortable : false,width : 30,align : 'center'},
    			{field : 'reformtime',title : '整改时间',sortable : false,width : 40,align : 'center',
    				formatter : function(value, row, index) {
    	              	if(value!=null&&value!='') {
    	              		var datetime=new Date(value);
    	              		return datetime.format('yyyy-MM-dd hh:mm:ss');
    			...
    			{field : 'reformphoto',title : '整改附件',sortable : false,width : 40,align : 'center',
    				formatter : function(value, row, index) {
        				var content="";
                      	if(value!=null&&value!='') {
                      		var arr1=value.split("||");
...
function view() {
	...
	openDialogView("查看隐患排查治理记录",ctx + "/yhpc/yhpcjl2/view/" + row.id,"900px","525px","");

}

//查看进度
function viewjd(id) {
        openDialogView("查看隐患排查治理进度",ctx + "/yhpc/yhpcjl2/viewjd/" + id,"800px","400px","");
}
// 弹窗修改
function upd() {
    var row = dg.datagrid('getSelected');
    ...
    openDialog("修改隐患排查治理", ctx + "/yhpc/yhpcjl2/update/" + row.id, "900px","525px","");
}
...
function del(){
	...
	top.layer.confirm('删除后无法恢复您确定要删除？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/yhpc/yhpcjl2/delete/"+ids,
