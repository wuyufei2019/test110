...
<head>
	...
	<meta name="decorator" content="default"/>
      <script type="text/javascript" src="${ctx}/static/model/js/targetmanger/mbzb/mbzp/qy_index.js?v=1.1"></script>
</head>
<body >
<!-- 工具栏 -->
<div id="target_mbzp_tb" style="padding:5px;height:auto">
	<form id="searchFrom" action="" style="margin-bottom: 8px;" class="form-inline pull-left">				
	<div class="form-group">	
   <c:if test="${ usertype eq 'isbloc' }">
        <input type="text"  name="target_mbzp_qyname" style="height:30px" class="easyui-textbox"
               data-options="width:200,prompt: '企业名称'" />
    </c:if>
	<input type="text"  name="target_mbzp_tname" style="height:30px" class="easyui-textbox" data-options="prompt: '指标名称'"/>																        										
	<input type="text" name="target_mbzp_year"id="target_mbzp_year" class="easyui-combobox"  style="width:100px;height: 30px;" 
   data-options="editable: false ,prompt:'年份',panelHeight:'120',valueField: 'year',textField: 'year' ">
    <input name="target_mbzp_quarter" type="text" class="easyui-combobox" style="height: 30px;width:100px;"
    ...
    <shiro:hasRole name="companyadmin">
    	<shiro:hasPermission name="target:mbzp:viewall">
    		<input name="target_mbzp_deptname" type="text" class="easyui-combobox" style="height: 30px;width:150px;"
   ...
   <div class="pull-left">
	...
	<div class="col-sm-12">
		<div class="pull-left">
            ...
            <shiro:hasPermission name="target:mbzp:update">
                <button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="upd()" title="修改"><i class="fa fa-file-text-o"></i> 修改</button> 
            </shiro:hasPermission>
            <shiro:hasPermission name="target:mbzp:delete">
               <button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="del()" title="删除"><i class="fa fa-trash-o"></i> 删除</button> 
            </shiro:hasPermission>
			<%-- <shiro:hasPermission name="target:mbzp:view">
        		<button class="btn btn-white btn-sm" data-toggle="tooltip" data-placement="left" onclick="view()" title="查看"><i class="fa fa-search-plus"></i> 查看</button> 
        	</shiro:hasPermission> --%>
        	<%-- <shiro:hasPermission name="target:mbzp:export">
...
<!--             <button class="btn btn-white btn-sm " data-toggle="tooltip" data-placement="left" onclick="statistics()" title="统计"><i class="fa fa-bar-chart"></i> 统计</button>
 -->         
        	<%-- <shiro:hasPermission name="target:mbzp:exin">
				<button class="btn btn-white btn-sm " data-toggle="tooltip" data-placement="left" onclick="openImportDialog('${ctx}/target/mbzp/exinjump','${ctx}/target/mbzp/exin','${ctx}/static/templates/指标信息导入模板.xls')" title="导入"><i class="fa fa-folder-open-o"></i> 导入</button>
			</shiro:hasPermission> --%>
...
$(function(){
	...
	$("#target_mbzp_year").combobox("loadData", data);
});

//弹窗增加
function init(id) {
	openDialog("导入季度自评信息",ctx+"/target/mbzp/create","800px", "60%","");
}
...
function del(){
	...
	top.layer.confirm('删除后无法恢复您确定要删除？', {icon: 3, title:'提示'}, function(index){
		$.ajax({
			type:'get',
			url:ctx+"/target/mbzp/delete/"+ids,
...
function upd(){
	...
	openDialog("修改目标自评信息",ctx+"/target/mbzp/update/"+row.id,"800px", "350px","");
	
}
//统计页面弹窗
function statistics(){
	openDialogView("统计分析",ctx+"/target/mbzp/statistics","50%", "100%","");
...
function view(){
	...
	openDialogView("查看目标自评信息",ctx+"/target/mbzp/view/"+row.id,"800px", "400px","");
...
function fileexport(){
	...
	layer.open({
        ...
        maxmin: false, 
        shift: 1,
	    content: ctx+'/target/mbzp/colindex',
		btn: ['导出'],
