			...
			<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer" >
			  <tbody>
				<tr >
					...
					<td class="width-85" >
						<input id="M16" name="M16" class="easyui-textbox" style="width: 100%;height: 80px;" value="" data-options="multiline:true,validType:'length[0,500]'" />
					</td>
				</tr>
			    ...
			    <tr>
				  ...
				  <td class="width-80" colspan="3">
					  <div id="uploader_ws_m16_1">
				      <div id="m16_1fileList" class="uploader-list" ></div>
				      <div id="imagePicker">选择图片</div>
				      </div> 
				  </td>
			    </tr>
								
				<input type="hidden" name="id" value="${lsydid }" />
				</tbody>
			</table>	
      </form>
<script type="text/javascript">
	...
	uploader.on( 'uploadSuccess', function( file ,lsyd) {
		...
		if(lsyd.error==0){
			...
			var newurl=lsyd.url+"||"+lsyd.fileName;
			var $input = $('<input id="input_'+file.id+'" type="hidden" name="M16_1" value="'+newurl+'"/>');
			
			$('#uploader_ws_m16_1').append( $input );
			uploadImgFlag=true;//存在上传图片
			
		}else{
			layer.msg(lsyd.message,{time: 3000});
		}
	});
	
	if('${action}' == 'updateSub'){
		var zpUrl = '${lsyd.m16_1}';
		if(zpUrl!=null&&zpUrl!=''){
			...
			for(var i=0;i<arry.length;i++){
			    ...
			    $list.append( $li );
			    var $input = $('<input id="input_'+id+'" type="hidden" name="M16_1" value="'+arry[i]+'"/>');
				$('#uploader_ws_m16_1').append( $input );
			}
		}
		isuploadImg();
