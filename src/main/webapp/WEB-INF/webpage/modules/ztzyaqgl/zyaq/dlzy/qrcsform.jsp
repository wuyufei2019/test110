			 ...
			 <tbody>
			  ...
			  <tr>
				  ...
				  <td class="width-80" colspan="3">
					  <div id="uploader_ws_m18_2">
				      <div id="m18_2fileList" class="uploader-list" ></div>
			   ...
			   <input type="hidden" name=dlid value="${id1 }" />
			<tbody>
		</table>
     </form>
...
function doSubmit(){
	isuploadImg();
	if(!uploadImgFlag){
		layer.open({title: '提示',offset: 'auto',content: '未上传图片，请上传！',shade: 0 ,time: 2000 });
		return;		
	}
...
$(function(){ 
    ...
    //图片上传成功，显示预览图
    uploader3.on( 'uploadSuccess', function( file ,dlzy) {
    	$.jBox.closeTip();
...
uploader.on( 'uploadSuccess', function( file ,dlzy) {
	...
	if(dlzy.error==0){
		...
		$('#uploader_ws_m18_2').append( $input );
