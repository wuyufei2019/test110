			...
			<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer" >
			  <tbody>
				<tr >
					...
					<td class="width-85" >
						<input id="M12" name="M12" class="easyui-textbox" style="width: 100%;height: 80px;" value="" data-options="multiline:true,validType:'length[0,500]'" />
					</td>
				</tr>
				...
				<input type="hidden" name="id" value="${dlzyid }" />
				</tbody>
			</table>
      </form>
<script type="text/javascript">
	...
	uploader.on( 'uploadSuccess', function( file ,dlzy) {
		...
		if(dlzy.error==0){
			...
			var newurl=dlzy.url+"||"+dlzy.fileName;
			...
			layer.msg(dlzy.message,{time: 3000});
		}
	});

	if('${action}' == 'updateSub'){
		var zpUrl = '${dlzy.m12_1}';
		if(zpUrl!=null&&zpUrl!=''){
			...
			for(var i=0;i<arry.length;i++){
			    ...
			    $list.append( $li );
			    var $input = $('<input id="input_'+id+'" type="hidden" name="M12_1" value="'+arry[i]+'"/>');
				$('#uploader_ws_m12_1').append( $input );
			}
		}
		isuploadImg();
