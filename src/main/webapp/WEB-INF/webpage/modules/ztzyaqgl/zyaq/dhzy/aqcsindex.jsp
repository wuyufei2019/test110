...
<head>
	...
	<script type="text/javascript" src="${ctx}/static/model/js/ztzyaqgl/zyaq/dhzy/aqcsindex.js?v=1.0"></script>
</head>
<body >
...
<table id="csdata" class="table table-bordered  table-condensed dataTables-example dataTable no-footer" >
	 <tbody>
		<tr>
			...
			<td class="width-70" >
				<div id="uploader_ws_M23_4">
			    <div id="M23_4fileList" class="uploader-list" ></div>
		...
		<tr>
			...
			<td class="width-70" >
				<div id="uploader_ws_M23_5">
			    <div id="M23_5fileList" class="uploader-list" ></div>
...
<script type="text/javascript">
	...
	if(type==0){
		...
		top.layer.confirm('是否编制安全措施？', {icon: 3, title:'提示'}, function(index){
			if(flag) {
				...
				var aqjd="";
				var sgfa="";
				$("input[name='M23_4']").each(function(j,item){
					aqjd+=item.value+",";
				});
				aqjd=aqjd.substr(0,aqjd.length-1)
				$("input[name='M23_5']").each(function(j,item){
					sgfa+=item.value+",";
				});
				sgfa=sgfa.substr(0,sgfa.length-1)
				$.ajax({
					type: 'get',
					url: ctx + "/ztzyaqgl/dhzy/createAqcs",
					data: {
	...
	$list = $('#M23_4fileList'); //文件上传
	$list2 = $('#M23_5fileList'); //文件上传
	...
	fileuploader.on( 'uploadSuccess', function( file ,res) {
		...
		if(res.error==0){
			...
			var $input = $('<input id="input_'+file.id+'" type="hidden" name="M23_4" value="'+newurl+'"/>');
			
			$('#uploader_ws_M23_4').append( $input );
		}else{
	...
	fileuploader2.on( 'uploadSuccess', function( file ,res) {
		...
		if(res.error==0){
			...
			var $input = $('<input id="input_'+file.id+'" type="hidden" name="M23_5" value="'+newurl+'"/>');
			
			$('#uploader_ws_M23_5').append( $input );
		}else{
			layer.msg(res.message,{time: 3000});
		}
	});
