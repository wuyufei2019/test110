			...
			<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer" >
			  <tbody>
				<tr >
					...
					<td class="width-85" >
						<input id="M13" name="M13" class="easyui-textbox" style="width: 100%;height: 80px;" value="" data-options="multiline:true,validType:'length[0,500]'" />
					</td>
				</tr>
				...
				<input type="hidden" name="id" value="${dtzyid }" />
				</tbody>
			</table>
      </form>
<script type="text/javascript">
	...
	uploader.on( 'uploadSuccess', function( file ,dtzy) {
		...
		if(dtzy.error==0){
			...
			var newurl=dtzy.url+"||"+dtzy.fileName;
			var $input = $('<input id="input_'+file.id+'" type="hidden" name="M13_1" value="'+newurl+'"/>');

			$('#uploader_ws_m16_1').append( $input );
			uploadImgFlag=true;//存在上传图片

		}else{
			layer.msg(dtzy.message,{time: 3000});
		}
	});

	if('${action}' == 'updateSub'){
		var zpUrl = '${dtzy.m16_1}';
		if(zpUrl!=null&&zpUrl!=''){
			...
			for(var i=0;i<arry.length;i++){
			    ...
			    $list.append( $li );
			    var $input = $('<input id="input_'+id+'" type="hidden" name="M13_1" value="'+arry[i]+'"/>');
				$('#uploader_ws_m16_1').append( $input );
			}
		}
		isuploadImg();
