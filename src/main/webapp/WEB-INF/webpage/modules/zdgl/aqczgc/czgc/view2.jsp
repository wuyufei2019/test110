...
<div class="shadow">
	<div style="padding: 30px 35px;">
	  	<div  class="fltitle" >${czgc.m1 }</div>
	  	<div  class="intro" >发布日期: <fmt:formatDate pattern="yyyy-MM-dd" value="${czgc.m4 }" /></div>
	  	<hr />
		<br />
	  	<div >
	  	<div>  
	  	 <c:if test="${not empty czgc.m16}">
				...
				<c:if test="${not empty czgc.m6}">
					<c:set var="url" value="${fn:split(czgc.m6, '||')}" />
						<div  style="margin-bottom: 10px;">
							...
							<img src="${ctxStatic}/model/images/p.png"><a style="color:#337ab7;text-decoration:none;cursor:pointer;margin-right:20px;" onclick="window.open('${czgc.m15}')">${url[1]}.pdf</a>
						</div>
				</c:if>
			</div>
