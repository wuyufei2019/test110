...
<body>
     <form id="inputForm" action="${ctx}/zdgl/glzd/${action}"  method="post" class="form-horizontal" >
			<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer" >
			  <tbody>
				<tr >
					<td class="width-15 active"><label class="pull-right">制度编号：</label></td>
					<td class="width-35" colspan="3">
						<input id="M2" name="M2" class="easyui-textbox" style="width: 100%;height: 30px;" value="${glzd.m2 }" data-options="required:'true',validType:'length[0,100]'" />
					</td>
				</tr>
				<tr >
					<td class="width-15 active"><label class="pull-right">制度名称：</label></td>
					<td class="width-35" colspan="3">
						<input id="M1" name="M1" class="easyui-textbox" style="width: 100%;height: 30px;" value="${glzd.m1 }" data-options="required:'true',validType:'length[0,100]'" />
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">版本号：</label></td>
					<td class="width-35"><input id="M3" name="M3" style="width: 100%;height: 30px;" class="easyui-textbox" value="${glzd.m3 }" data-options="validType:'length[0,100]'"/></td>
					<td class="width-15 active"><label class="pull-right">发布日期：</label></td>
					<td class="width-35" ><input id="M4" name="M4" class="easyui-datebox" style="width: 100%;height: 30px;" value="${glzd.m4 }" data-options="editable:false" /></td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">制度正文：</label></td>
					<td class="width-35" colspan="3">
						<input type="text" name="M5" class="easyui-textbox" value="${glzd.m5 }"  data-options="multiline:true" style="width: 100%;height: 80px;" />
					</td>
				</tr>
				<tr>
					<td class="width-15 active"><label class="pull-right">附件（单文件）：</label></td>
					<td colspan="3">
					<input type="hidden" id="M6" name="M6" value="${glzd.m6}" />
					<div id="uploader_glzd_m6">
				...
				<tr>
					...
					<td class="width-35">
						<input id="M7" name="M7" style="width: 100%;height: 30px;" class="easyui-combobox" value="${glzd.m7 }" 
					...
					<td class="width-35">
						<input id="M8" name="M8" style="width: 100%;height: 30px;" class="easyui-combobox" value="${glzd.m8 }" 
							data-options="validType:'length[0,250]',panelHeight:'150px',editable:false,multiple:true,valueField:'id',textField:'text',url:'${ctx }/system/department/deptjson'"/>
					</td>
				</tr>
				...
				<c:if test="${action != 'create'}">
					...
					<c:if test="${type eq 'pz'}">
						...
						<input type="hidden" name="M9" value="${glzd.m9}" />
						<input type="hidden" name="M10" value="${glzd.m10}" />
						<input type="hidden" name="M11"
							value="<fmt:formatDate value="${glzd.m11}" pattern="yyyy-MM-dd HH:mm:ss"/>" />
					</c:if>
				</c:if>
				
				<c:if test="${not empty glzd.ID}">
					<input type="hidden" name="ID" value="${glzd.ID}" />
					<input type="hidden" name="ID1" value="${glzd.ID1}" />
					<input type="hidden" name="S1"
						value="<fmt:formatDate value="${glzd.s1}" pattern="yyyy-MM-dd HH:mm:ss"/>" />
					<input type="hidden" name="S3" value="${glzd.s3}" />
					<input type="hidden" name="userid" value="${glzd.userid}" />
				</c:if>
				</tbody>
			</table>
       </form>
