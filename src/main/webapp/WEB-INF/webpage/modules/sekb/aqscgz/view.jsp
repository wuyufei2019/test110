    ...
    <div id="printContainer"></div>
				...
				<c:if test="${not empty res.m5}">
					<c:set var="url" value="${fn:split(res.m5, '||')}" />
						<div  style="margin-bottom: 10px;">
							...
							<img src="${ctxStatic}/model/images/p.png"><a style="color:#337ab7;text-decoration:none;cursor:pointer;margin-right:20px;" onclick="window.open('${res.m6}')">${url[1]}.pdf</a>
						</div>
				</c:if>
			</div>
			<label>附件：</label>
			<div>
				<c:if test="${not empty res.m8}">
					<c:set var="url" value="${fn:split(res.m8, '||')}" />
						<div  style="margin-bottom: 10px;">
					 		<c:if test="${not empty res.m8}">
