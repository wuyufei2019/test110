...
function saveMarker(){
    if(fenceState == 1) {
        var mappoint =[] ;
        console.info(newFenceData);
        for (var i = 0; i < newFenceData.length; i++) {
            mappoint.push({x : newFenceData[i].x,y : floor,z : newFenceData[i].y})
        ...
        layer.confirm('确认保存该电子围栏？', {icon: 3, title:'提示'}, function(index){
            $.ajax({
                ...
                data: {"wlid":wlId,"mappoint":JSON.stringify(mappoint).toString(),"floor":floor,"xbids":xbids,"type":"2"},
                success: function(data){
