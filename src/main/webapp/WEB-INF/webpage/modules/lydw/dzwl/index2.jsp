    ...
    function doSaveTip() {
        layer.confirm('保存该电子围栏？', {
            ...
            if(parent && parent.length<2){
                data ={type:1,ID:points.thingid,mappoint: JSON.stringify(mappoint).toString(),floor:1,floorname:'',building:'',buildingname:'',points:JSON.stringify(points).toString()};
            }else{
                data = {type:1,ID:points.thingid,mappoint: JSON.stringify(mappoint).toString(),floor:parent[0].id,floorname:parent[0].name,building:parent[1].id,buildingname:parent[1].name,points:JSON.stringify(points).toString()}
            }
