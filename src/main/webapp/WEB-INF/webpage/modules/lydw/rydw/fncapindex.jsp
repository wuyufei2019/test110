...
<head>
    ...
    <script type="text/javascript" src="${ctxStatic}/fengmap/js/jquery-1.12.0.min.js"></script>
...
//创建转换器
    ...
    window.onload = function() {
                ...
                if (polygonMarkerId != -1) {
                    if(polygonMarkerId == -2) {
                        ...
                        for(var i=0;i<fenceDataList.length;i++){
                            addPmMarker(fenceDataList[i].fence,fenceDataList[polygonMarkerId].fenceId,focus);
                        }
                    }else{
                        addPmMarker(fenceDataList[polygonMarkerId].fence,fenceDataList[polygonMarkerId].fenceId,focus);
                    }
                }
            });
        });
        ...
        map.on('mapClickNode', function(event) {
            if (event.nodeType == fengmap.FMNodeType.IMAGE_MARKER) {
                ...
                //显示弹框
                if (contentStyle(popmarkerInfo)) {
                    showBounced(event.target.x,event.target.y,event.target.groupID,contentStyle(popmarkerInfo));
                }
    ...
    function updatePersonMarkerInfo(data) {
        if (data != null) {
            ...
            var personCoord = jQuery.parseJSON(data)[0];
            var file = personCoord.tagid;//设备id
            var flag = false;//该设备是否已存在
            var loc = {'x':personCoord.position[0],'y':personCoord.position[2]};
            var mapCoord = trasformer.transform(loc);
            personCoord.position[0] = mapCoord.x;
            personCoord.position[2] = mapCoord.y;
            for (var i = 0; i < personMarkerInfo.length ; i++) {

                if (personMarkerInfo[i].file == file) {
                    flag = true;
                    personMarkerInfo[i].x = personCoord.position[0];
                    personMarkerInfo[i].y = personCoord.position[2];
                    personMarkerInfo[i].z = personCoord.position[1];

                    var popmarkerInfo = personMarkerInfo[i].popmarkerInfo.split("_");//信息
                    //更新蓝牙定位日期信息
                    var newInfo = popmarkerInfo[0]+"_"+popmarkerInfo[1]+"_"+popmarkerInfo[2]+"_"+ dateFtt("yyyy-MM-dd hh:mm:ss",new Date(personCoord.time));
                    personMarkerInfo[i].popmarkerInfo = newInfo;


                    markerMap["_"+personMarkerInfo[i].id]["x_"] = personMarkerInfo[i].x;
                    markerMap["_"+personMarkerInfo[i].id]["y_"] = personMarkerInfo[i].y;
                    markerMap["_"+personMarkerInfo[i].id]["z_"] = personMarkerInfo[i].z;
    ...
    function transformData(personCoords) {
        var popmarkerInfo = personCoords.name+"_"+personCoords.depname+"_"+personCoords.carrer;
        var loc = {'x':personCoords.position[0],'y':personCoords.position[2]};
        var mapCoord = trasformer.transform(loc);
        personCoords.position[0] = mapCoord.x;
        personCoords.position[2] = mapCoord.y;
        var personCoord = {
            x : personCoords.position[0],
            y : personCoords.position[2],
            z : personCoords.position[1],
            name: personCoords.name,
            id : personCoords.id,
            file : personCoords.tagid,
            popmarkerInfo : popmarkerInfo
        }
        return personCoord;
    }
        ...
        if (data != null) {
            ...
            if (personCoordsLength > 0) {
                ...
                for (var i = 0; i < personCoordsLength; i++) {
                    var personCoord = transformData(personCoords[i]);
                    ...
                    if(Object.getOwnPropertyNames(markerMap).length>0){
                        ...
                        if(markerMap["_"+personCoord.id]){
                            markerMap["_"+personCoord.id].moveTo({
                                //设置imageMarker的x坐标
                                x: personCoord.x,
                                //设置imageMarker的y坐标
                                y: personCoord.y,
                            ...
                            markerMap["_"+personCoord.id].name_ = personCoord.popmarkerInfo;
                            // 将已存在的im对象放入新的集合中
                            markerMap_cz_id.push("_"+personCoord.id);
                        }else{
    ...
    function findPerson(personInpVal,state) {
        ...
        var isfind = false;
        for(var i = 0; i < personMarkerInfo.length; i++){
            if(personMarkerInfo[i].name.indexOf(personInpVal) >=0){
                isfind = true;
                var queryMarker = personMarkerInfo[i];
                console.info(queryMarker);
                ...
                //弹出框中显示的信息
                var popmarkerInfo=contentStyle(queryMarker.popmarkerInfo);
                //显示弹框
                showBounced(queryMarker.x,queryMarker.y,map.groupIDs[queryMarker.z],popmarkerInfo);
    ...
    //去除所有的标记点
    function removeim2() {
        if (personLayer != null && im2List.length>0) {
            for (var i = 0; i < im2List.length; i++) {
                personLayer.removeMarker(im2List[i]);
            }
        }
    }
     ...
     *展示信息处理
     * */
    function contentStyle(str) {
        if (str) {
            var popmarkerInfoArr = str.split("_");
            return '<div class="popMarkerDiv"><div style="height:30px;">'+popmarkerInfoArr[0]+'</div><div style="height:20px;font-size:12px">'+popmarkerInfoArr[1]+' - '+popmarkerInfoArr[2]+'</div></div>';
        }
        return '';
     ...
     * content:展示的数据
     * */
    function showBounced(x,y,group,content) {
        //信息框控件大小配置
            ...
            success: function(data){
                ...
                if(data.length > 0 && data != null){
                    for (var i = 0; i < data.length; i++) {
                        var index = i;
                        var fenceData = [];
                        var ponits = $.parseJSON(data[i].mappoint) ;
