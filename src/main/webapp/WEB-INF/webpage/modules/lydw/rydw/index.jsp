...
function getPersonCoords() {
		...
		success : function(data) {
			if (data != null) {
				...
				if (personCoordsLength > 0) {
					...
					if(personMarkerInfo!=null && personMarkerInfo.length>0){
						var length = personMarkerInfo.length;
						if(length>0 && length>50){
							addMarkersBatch(0,50,length,personMarkerInfo);
						}
						else if(length>0 && length<=50){
							addMarkersBatch(0,length,length,personMarkerInfo);
						}
					}
				}
			}
    ...
    updateTimer = window.setInterval(updateMarkers, 10000);
}
