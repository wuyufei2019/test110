		...
		<div class="form-group">
			...
			<input name="type" id="type" class="easyui-combobox" style="height: 30px;"
				   data-options="prompt: '气体类型',editable:false,panelHeight:'auto',data:[
				   	   {value:'有毒气体',text:'有毒气体'},
				   	   {value:'可燃气体',text:'可燃气体'}]" />
...
<script>
	...
	$(function(){
		dg=$('#zxjkyj_bjxx_dg').datagrid({
			nowrap:false,
			method: "post",
			url:ctx+'/zxjkyj/bjxx/ndbjlist',
			queryParams: {sigid: sigid, type: type},
			fit : true,
			fitColumns : true,
			selectOnCheck:false,
			nowrap: false,
			...
			columns:[[
				...
				{field:'value',title:'气体报警浓度',sortable:false,width:80,align:'center',
					formatter : function(value, row, index) {
	...
	// 		if (date > $('#endtime').datebox('getValue')) {
	// 			layer.msg('');
	// 		}
	// 	}
	// })
