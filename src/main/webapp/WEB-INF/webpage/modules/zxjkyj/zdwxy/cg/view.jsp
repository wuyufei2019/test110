...
.r_num {color:#fffda0; font-size:14px; position:absolute; top:50%; margin-top:-25px; text-align:center; width:100%;}
  ...
  <div style="width: 100%;text-align: center;"><h4><span style="color: #f8ac59;font-size: 24px;">${qiye.m1}</span></h4></div>
   <div class="wdytyl_pic_list" align="center" style="margin: 0px auto;">
   		...
   		<c:forEach items="${cllist}" var="st" varStatus="status">
			<c:if test="${st.lx==1}">
				<div style="float: left;position:relative;margin: 10px 10px 0px 10px;" onclick="showCgsssjXq(${st.id})">
					<div class="wdytyl_cg_bg1">
						...
						物质:${st.wl}<br/>
						温度:<fmt:formatNumber value="${st.wd}" maxFractionDigits="2"/>℃<br/>
						压力：<fmt:formatNumber value="${st.yl}" maxFractionDigits="3"/>kPa<br/>
						液位:<fmt:formatNumber value="${st.yw}" maxFractionDigits="2"/>m<br/></div>
  ...
  <script type="text/javascript">
	...
	// 查看储罐实时数据信息
	function showCgsssjXq(tankid) {
		openDialogView("查看储罐实时数据信息",ctx+"/zxjkyj/zdwxycg/viewcgsssjxq/"+tankid,"800px", "400px","");
	}
