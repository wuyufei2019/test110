...
.r_num {color:#fffda0; font-size:14px; position:absolute; top:50%; margin-top:-25px; text-align:center; width:100%;}
  ...
  <div style="width: 100%;text-align: center;"><h4><span style="color: #f8ac59;font-size: 24px;">${qiye.m1}</span></h4></div>
   <div class="wdytyl_pic_list" align="center" >
   		<c:forEach items="${cllist}" var="st" varStatus="status">
			...
			<div style="float: left;position:relative;margin: 10px 10px 0px 10px;border: 2px #f2f2f2 solid;padding: 20px;" onclick="showQtsssjXq(${st.id},'${st.qttype}')">
				<!-- 百分比大于等级1时显示静态，否则显示动态 -->
				<div class="<c:if test="${st.bjsj != null}">wdytyl_cg_bg1</c:if><c:if test="${st.bjsj eq null}">wdytyl_cg_bg2</c:if>">
				
					<div class="qtinfor">
					<span style="font-weight: bold;"></span><label style="color:#c2d1e5">${st.qtmc}</label><br/>
					 
					<span><fmt:formatNumber value="${st.ssnd}" maxFractionDigits="2"/></span><br/>
					...
					<label style="color:#337ab7"><%-- 传感器编号:${st.cgqbh} --%></label><br/> 
  ...
  <script type="text/javascript">
	...
	// 查看气体实时数据信息
	function showQtsssjXq(pointid, qttype) {
		openDialogView("查看气体实时数据信息",ctx+"/zxjkyj/zdwxyqt/viewxq/"+pointid+"/"+qttype,"800px", "400px","");
	}
