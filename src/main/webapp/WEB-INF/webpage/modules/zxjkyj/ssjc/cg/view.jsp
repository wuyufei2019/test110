		...
		<c:if test="${st.m2==1}">
			<div style="float: left;position:relative;margin: 10px 10px 0px 10px;"  >
				<div class="wdytyl_cg_bg1">
					...
					<div style=" position:relative; top:0px; left:0px;z-index:2;color: #666;cursor: pointer;width: 220px;padding: 85px 0px;" onmouseover="showInfor('${status.count}','${st.m7}',0,event)" onmouseout="hideInfor('${status.count}')">
					物质:${st.m7}<br/>
					储量:<fmt:formatNumber type="number" value="${st.reserves}" maxFractionDigits="2"/>m³<br/>
					比例:<fmt:formatNumber value="${st.percent*100}" maxFractionDigits="2"/>%<br/>
		...
		<c:if test="${st.m2==2}">
			<div style="float: left;position:relative;margin: 10px 10px 0px 10px;" >
				<div class="wdytyl_cg_bg2">
					...
					<div style="position:relative; top:18px; left:0px;z-index:2;color: #666;cursor: pointer;width: 400px;padding: 45px 0px 100px;"onmouseover="showInfor('${status.count}','${st.m7}',1,event)" onmouseout="hideInfor('${status.count}')">
					物质:${st.m7}<br/>
					储量:<fmt:formatNumber value="${st.reserves}" maxFractionDigits="2"/>m³<br/>
					比例:<fmt:formatNumber value="${st.percent*100}" maxFractionDigits="2"/>%<br/>
					温度:<fmt:formatNumber value="${st.temperature}" maxFractionDigits="2"/>℃<br/>
					压力：<fmt:formatNumber value="${st.pressure}" maxFractionDigits="3"/>MPa<br/></div>
		...
		<c:if test="${st.m2==3}">
			<div style="float: left;position:relative;margin:10px 10px 0px 10px;" >
				<div class="wdytyl_cg_bg3">
					...
					<div style="position:relative; top:0px; left:0px;z-index:2;color: #666;cursor: pointer;width: 250px;padding: 70px 0px;" onmouseover="showInfor('${status.count}','${st.m7}',0,event)" onmouseout="hideInfor('${status.count}')">
					物质:${st.m7}<br/>
					储量:<fmt:formatNumber value="${st.reserves}" maxFractionDigits="2"/>m³<br/>
					比例:<fmt:formatNumber value="${st.percent*100}" maxFractionDigits="2"/>%<br/>
					温度:<fmt:formatNumber value="${st.temperature}" maxFractionDigits="2"/>℃<br/>
